...
# 通常戦モデル(予想5頭)は2026-07-29の再モデリングでBOX4回収率検証モデルと統一した。
# winner_box4_nar.json(等重み14シグナル、重み探索なし)が正本で、レースカードの予想5頭
# (predictions_nar_{date}.csv、predict_top5_nar.py生成)・予想5頭BOX回収率検証
# (confidence_sweep_baseline_nar.csv)もすべて同じ重みを使う。
winner = json.loads((DATA_DIR / "winner_box4_nar.json").read_text(encoding="utf-8"))
...
WEIGHT_META_EXTRA = {
    ...
    "weight": ("直近馬体重の増減幅(v2)", "当日再取得したbias_horse_weightの増減幅。プラス(増加)ほど高評価、かつ絶対値が小さいほど高評価の2成分をブレンド。専門家レビューでは「馬体重増加が必ずしも好材料とは限らない」との実務上の懸念があり、実際に300パターン探索でも重みは0.6%とごく小さい"),
}
...
conf_tabs_css_box5, box_section = build_tabbed_box_section(
    ...
    lede=(
        ...
        f"2026-07-29、ユーザー指示により<b>{esc(MODEL_LABEL)}</b>(等重みを廃止し、"
        "全17シグナルに差のある重みを持たせた300パターン自由探索)へ更新済みです。"
        "この表の数字は探索に使ったのと同じ126レースでの実測(in-sampleで楽観的)である点に"
        "注意してください。誠実な汎化性能の推定値・専門家レビューの詳細は"
...
winner_box4 = winner  # 通常戦モデル(予想5頭)と同一のwinner_box4_nar.json・同一の重み
...
_b4_edge = BOX4_BIAS.get("true_edge_pt", 0.0)
_b4_edge_sd = BOX4_BIAS.get("true_edge_sd", 0.0)
_b4_win = BOX4_BIAS.get("win_rate", 0.0) * 100
_b4_search = winner_box4.get("search300_2026_07_29", {})
_b4_nested_excess = _b4_search.get("nested_lobo_oof", {}).get("excess", 0.0)
_b4_baseline_excess = _b4_search.get("baseline_equal_weight_14", {}).get("excess", 0.0)
_b4_review = _b4_search.get("review_process", {}).get("review_summary", {})
conf_tabs_css_box4, box_section_box4 = build_tabbed_box_section(
    "confidence_sweep_box4_nar.csv", "sec-box4", "b4", "予想4頭BOX 回収率検証",
    lede=(
        "<b>2026-07-29、ユーザー指示により3回再モデリングしたモデル</b>による検証です。"
        "以前は「自由探索は126レースでは有害」という実測結果(楽観バイアス+28.1pt、選ぶことの"
        "真の価値-2.19pt)に基づき、探索をやめて生存14シグナルの等重みに固定していました。"
        "今回ユーザーから明示的に「等重みをやめ、シグナルに重みを持たせ、パターンを300に"
        "増やして検証してほしい」との指示があり、新規3シグナル(直近5走の1着とのタイム差・"
        "クラス×人気順補正・直近馬体重の増減幅)を加えた全17シグナルで300パターンの"
        "自由探索を実施しました。"
        "<b style=\"color:#c0392b\">重要な注意: この表の数字(全126レース・高確信度タブとも)は"
        "探索に使ったのと同じデータでの実測(in-sample)であり、楽観的です。"
        "12ブロックの交差検証による誠実な推定(Nested LOBO OOF)では市場差"
        f"{_b4_nested_excess:+.2f}pt(現行の等重みモデルは{_b4_baseline_excess:+.2f}pt)と、"
        "実際にはむしろ悪化する可能性が高いという測定結果が出ています。</b>"
    ...
    method_note=(
        ...
        "比率が大きい順にレースを選んでいます(2026-07-29、確信度較正の検証で採用。詳細は"
        "下記＜確信度指標の較正＞参照)。NARは1場あたり最大12レースの開催があるため、"
        "JRAのN=5〜10ではなくN=5〜12まで対象にしています。"
        "＜300パターン自由探索の実測結果＞ 全17シグナル(生存14+新規3)にDirichlet分布で"
        "300パターンの重みを生成し、全126レースで最良のものを選んだところ、学習データ"
        f"そのものでの市場差は{_b4_excess:+.2f}pt(現行の等重みは{_b4_baseline_excess:+.2f}pt)"
        "という大幅改善に見えました。ところがこれは「そのデータで選んだ」ことによる楽観を"
        "含むため、開催日×競馬場の12ブロックについて「1ブロックを除いた残りで300パターン"
        "探索・選択し、除いたブロックで評価する」交差検証(Nested LOBO)を実施したところ、"
        f"誠実な汎化性能推定は市場差{_b4_nested_excess:+.2f}ptとなり、現行の等重みモデル"
        f"({_b4_baseline_excess:+.2f}pt)は言うに及ばず市場そのものも大きく下回りました。"
        "＜選択バイアス診断＞ ブロック半分割×200反復の結果、選んだ側の平均"
        f"{BOX4_BIAS.get('selected_side', 0):.1f}%に対し未使用側では"
        f"{BOX4_BIAS.get('unseen_side', 0):.1f}%まで落ち(楽観バイアス"
        f"{BOX4_BIAS.get('optimism_pt', 0):+.1f}pt)、選ぶことの正味の価値は"
        f"{_b4_edge:+.2f}pt(標準偏差{_b4_edge_sd:.2f})、平均を上回る確率は{_b4_win:.0f}%と"
        "コイン投げ以下でした。"
        "＜専門家レビューと採用の経緯＞ "
        f"{esc(_b4_review.get('box_n_4', ''))} "
        "この推奨をユーザーに提示した上で改めて確認したところ、ユーザーはこの測定結果を"
        "承知の上で「指示通り採用する」ことを明示的に選択したため、レビューの推奨ではなく"
        "ユーザーの判断により本モデル(300パターン探索・差のある重み)を採用しています。"
        "新シグナルの設計自体にも、専門家・エンジニア双方から改善余地の指摘があります"
...
_BOX3_BIAS = winner_box3.get("selection_bias_measured", {})
_b3_edge = _BOX3_BIAS.get("true_edge_pt", 0.0)
_b3_edge_sd = _BOX3_BIAS.get("true_edge_sd", 0.0)
_b3_win = _BOX3_BIAS.get("win_rate", 0.0) * 100
_b3_search = winner_box3.get("search300_2026_07_29", {})
_b3_nested_excess = _b3_search.get("nested_lobo_oof", {}).get("excess", 0.0)
_b3_baseline_excess = _b3_search.get("baseline_equal_weight_14", {}).get("excess", 0.0)
_b3_review = _b3_search.get("review_process", {}).get("review_summary", {})
_b3_boot95 = _b3_search.get("bootstrap_full_population_95ci", {})
conf_tabs_css_box3, box_section_box3 = build_tabbed_box_section(
    "confidence_sweep_box3_nar.csv", "sec-box3", "b3", "予想3頭BOX 回収率検証",
    lede=(
        "<b>2026-07-29、ユーザー指示により3回再モデリングしたモデル</b>による検証です。"
        "以前は競馬専門家・シニアエンジニアのレビューを踏まえ、新規候補(休養日数適性・"
        "斤量帯適性)をLOBO OOFで検証しつつ、探索は有害という知見から生存14シグナルの"
        "等重みに固定していました。今回ユーザーから明示的に「等重みをやめ、シグナルに重みを"
        "持たせ、パターンを300に増やして検証してほしい」との指示があり、新規3シグナル"
        "(直近5走の1着とのタイム差・クラス×人気順補正・直近馬体重の増減幅)を加えた全17"
        "シグナルで300パターンの自由探索を実施し、4頭BOXと同じパターン#157の重みを採用"
        "しています。"
        "<b>この表の数字は探索に使ったのと同じデータでの実測(in-sample)であり、12ブロックの"
        f"交差検証(Nested LOBO OOF)による誠実な推定は市場差{_b3_nested_excess:+.2f}pt"
        f"(現行の等重みモデルは{_b3_baseline_excess:+.2f}pt)です。</b>3頭まで絞ると3連複は"
        "「BOXした3頭がそのまま実際の1〜3着」でないと的中しない1点賭けになるため、"
        "5頭・4頭BOXより的中率は大きく下がる一方、当たった場合の倍率は跳ね上がります。"
        "タブで対象レースの絞り込み条件を切り替えられます。"
    ),
    method_note=(
        ...
        "を(1位−最下位)の幅で正規化した比率が大きい順にレースを選んでいます(2026-07-29、"
        "確信度較正の検証で採用。詳細は下記＜確信度指標の較正＞参照)。"
        ...
        "＜300パターン自由探索の実測結果＞ 全126レースで最良の1パターンを選んだ学習データ"
        f"そのものでの市場差は{_b3_excess:+.2f}pt(現行の等重みは{_b3_baseline_excess:+.2f}pt)"
        "でしたが、12ブロックのNested LOBO交差検証による誠実な推定は市場差"
        f"{_b3_nested_excess:+.2f}ptで、現行の等重みモデルよりわずかに良い程度にとどまります。"
        "ブロックブートストラップ95%信頼区間は"
        f"[{_b3_boot95.get('lo', 0):.1f}%, {_b3_boot95.get('hi', 0):.1f}%]"
        "(全126レース実測値ベース)。"
        "＜選択バイアス診断＞ ブロック半分割×200反復の結果、選ぶことの正味の価値は"
        f"{_b3_edge:+.2f}pt(標準偏差{_b3_edge_sd:.2f})、未使用側で300パターン平均を"
        f"上回る確率は{_b3_win:.0f}%でした。改善幅(+"
        f"{_b3_nested_excess - _b3_baseline_excess:.2f}pt)はこの標準偏差に比べて小さく、"
        "統計的に有意な改善と断定はできません。"
        "＜専門家レビューと採用の経緯＞ "
        f"{esc(_b3_review.get('box_n_3', ''))} "
        "この推奨をユーザーに提示した上で改めて確認したところ、ユーザーはこれを承知の上で"
        "「指示通り採用する」ことを明示的に選択したため、レビューの推奨(保留)ではなく"
        "ユーザーの判断により本モデルを採用しています。旧pattern6・旧いinterval/kinryo検証の"
        "詳細な経緯は「予想3頭BOXモデルの内訳」を参照してください。"
...
<main>
  <header class="masthead">
    ...
    <details class="method">
      ...
      <p class="method-note">
        <b>2026-07-29、3回目の更新(ユーザー指示による方針転換)。</b>2回目の更新までは
        「薄いデータでの重み探索は成績を悪化させる」という実測に基づき、生存{len(MODEL_ALIVE)}個の
        シグナルを全て同じ重みで使う固定ルールにしていたが、ユーザーから明示的に「等重みを
        やめ、シグナルに重みを持たせ、パターンを300に増やして検証してほしい」との指示があり、
        新規3シグナル(直近5走の1着とのタイム差・クラス×人気順補正・直近馬体重の増減幅)を
        加えた全17シグナルで300パターンの自由探索を実施し、そこで選ばれた差のある重みを
        採用している。この方針転換の測定結果(交差検証による誠実な性能推定が現行の等重み
        モデルを下回るという結果を含む)・専門家レビューの詳細は「予想4頭BOXモデルの内訳」を
        参照。表示頭数のみ5頭のまま変えていない(4頭BOXは回収率検証専用に上位4頭で判定)。
    ...
    <details class="method">
      ...
      <p class="method-note" style="border-left:3px solid #c0392b; padding-left:0.8em;">
        <b style="color:#c0392b">重要: このモデルは、競馬専門家・シニアエンジニアの両レビューが
        「不採用・現行の等重み維持」を推奨したにもかかわらず、ユーザーが測定上のリスクを
        承知の上で明示的に採用を指示したモデルです。</b>
      </p>
      <p class="method-note">
        <b>経緯。</b>以前は「重み探索は126レースでは有害」という実測(選ぶことの正味の価値
        {BOX4_BIAS.get('true_edge_pt', 0):+.2f}pt、標準偏差{BOX4_BIAS.get('true_edge_sd', 0):.2f}、
        平均を上回る確率{BOX4_BIAS.get('win_rate', 0) * 100:.0f}%)に基づき、生存14シグナルの
        等重みに固定していた(市場差{_b4_baseline_excess:+.2f}pt)。
        2026-07-29、ユーザーから「等重みをやめ、シグナルに重みを持たせ、300パターンで
        検証してほしい」との明示的な指示があり、新規3シグナル
        (timediff・class_ninki・weight、詳細は上のリスト参照)を加えた全17シグナルで
        Dirichlet分布から300パターンの重みを生成し、全126レースで最良の1パターン(#157)を
        選んだ。
        <b>学習データそのものでの市場差は{_b4_excess:+.2f}ptと大幅改善に見えるが、
        開催日×競馬場の12ブロックで「1ブロックを除いた残りで300パターン探索・選択し、
        除いたブロックで評価する」交差検証(Nested LOBO)による誠実な汎化性能推定は
        市場差{_b4_nested_excess:+.2f}ptであり、現行の等重みモデルはおろか市場そのものも
        大きく下回る。</b>in-sampleとNested LOBO OOFの乖離(約
        {_b4_excess - _b4_nested_excess:.0f}pt)は、以前実測した選択バイアス(+28.1pt)と
        同規模であり、典型的な過学習と解釈するのが妥当というのがシニアエンジニアレビューの
        見解である。
        競馬専門家レビューは、class_ninkiの「1/人気順」という非線形性が人気上位同士の差を
        過大評価し下位同士を過小評価する歪みを生みやすいこと、weightの「馬体重増加=好材料」
        という単純化が競馬の実務(大幅増はむしろ仕上がり不足のサインであることが多い)と
        齟齬することを指摘した(実際、探索結果でもweightの重みは0.6%とごく小さい)。
        シニアエンジニアレビューは、5走加重平均の減衰関数の不明確さ、少数の外れ値による
        minmax正規化の歪みやすさに加え、timediffを検証ゲート(LOBO OOF二値採否)を通さず
        採用した判断について「サンプル増加の見込みだけで事前登録基準を後出しで緩和するのは
        多重検定リスクを伴う」と指摘した。
        両レビューとも「本番採用の統計的根拠はない、現行の等重みモデルを維持すべき」という
        結論で一致していたが、この推奨をユーザーに提示した上で改めて確認したところ、
        ユーザーはこの測定結果を承知の上で「指示通り採用する」ことを明示的に選択したため、
        レビューの推奨ではなくユーザーの判断により本モデルを採用している。
        {_factor_v2_note(BOX4_FACTOR_V2, base_label="基準")}
      </p>
    </details>
    ...
    <details class="method">
      ...
      <p class="method-note">
        <b>2026-07-29、3回目の更新(ユーザー指示による方針転換)。</b>
        以前は競馬専門家・シニアエンジニア2名のレビューを踏まえ、新規候補(休養日数適性・
        斤量帯適性)をLOBO OOFの二値採否で検証しつつ(いずれも基準を悪化させ不採用)、
        血統系統・馬場状態適性は実装・リークリスクの懸念から見送り、探索は有害という
        知見(旧pattern6と同型の自由探索で楽観バイアス+27.7pt・選ぶことの正味の価値
        +0.67pt・sd7.11)に基づき生存14シグナルの等重みに固定していた(市場差
        {_b3_baseline_excess:+.2f}pt)。
        2026-07-29、ユーザーから「等重みをやめ、シグナルに重みを持たせ、300パターンで
        検証してほしい」との明示的な指示があり、新規3シグナル(timediff・class_ninki・
        weight、詳細は上のリスト参照)を加えた全17シグナルで300パターンの自由探索を実施し、
        4頭BOXと同じパターン#157の重みを採用した。
        学習データそのものでの市場差は{_b3_excess:+.2f}ptだったが、12ブロックのNested LOBO
        交差検証による誠実な推定は市場差{_b3_nested_excess:+.2f}pt(現行の等重みモデルは
        {_b3_baseline_excess:+.2f}pt)で、改善幅(+{_b3_nested_excess - _b3_baseline_excess:.2f}pt)は
        選択バイアス診断の標準偏差({_b3_edge_sd:.2f})に比べて小さく、統計的に有意な改善とは
        言えない。両レビューとも「保留・実質現状維持」を推奨したが、ユーザーはこれを承知の
        上で採用を明示的に選択した。box4側で指摘された新シグナル設計への懸念(class_ninkiの
        非線形性、weightの実務との齟齬、timediffの検証ゲート回避)はbox3にも同様に当てはまる
        (詳細は「予想4頭BOXモデルの内訳」参照)。
