...
conf_tabs_css_box5, box_section = build_tabbed_box_section(
    ...
    lede=(
        ...
        "切り替えられます。"
        "<br><br>＜2026-08-20/21再検証(ユーザー依頼)＞ インターネット由来の勝率アップ手法"
        "(クラス降格・乗り替わり・持続タイム等)を新規5シグナルとして追加実装し(生存22本、"
        "目標50に対し正直な内訳は本文書冒頭参照)、データ増加(1385レース・127ブロック、"
        "前回253レース・23ブロックの約5.5倍)を踏まえ重み探索を再実施しました"
        "(統計学者・競馬予想家の2専門家による設計レビュー→実行→結果レビューの2段階)。"
        "box5は市場比で統計的に有意に悪化(Nested LOBO OOF市場差-10.81pt、95%CI完全に負)。"
        "box4/box3は表面上プラスでしたが、500パターン中の勝者が2位に2pt超差をつけて"
        "127分割全てで選ばれ続けたため、held-out検証のはずが実質的にin-sample評価に"
        "退化する測定アーティファクトと判明(選択バイアス診断の「選ぶことの真の価値」は"
        "box4/box3ともほぼゼロ=コイントス水準)。競馬場別・クラス別の異質性診断も実施しましたが、"
        "多重比較補正(Bonferroni、16区分)後に有意な区分は0でした(ただし大井・笠松・"
        "上級クラスの3区分は補正なしでは有意で、標本不足による判定保留の可能性があり継続監視)。"
        "以上により、5シグナル追加・500パターン探索とも本番重みには反映せず、17シグナル等重みを"
        "維持しています。"
    ),
    method_note=(
        "1点100円換算、単勝・複勝は5点、馬連・ワイド・3連複は10点、馬単は20点、3連単は60点。"
