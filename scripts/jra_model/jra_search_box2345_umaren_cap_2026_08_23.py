# -*- coding: utf-8 -*-
"""ユーザー依頼(2026-08-23続き2): 「馬連BOX５に広げて、馬連最適化パターンを探してください。
払い戻し2万円以上は引き続きお願いします」

先行実施のjra_search_box234_umaren_cap_2026_08_23.py(box_n=2,3,4)にbox_n=5を追加し、
box5/4/3/2の4サイズを揃える。box_nごとの探索ロジック・シード(SEED+BOX_N)は先行スクリプトと
完全に同一なので、box2/3/4の結果は数値的に再現される(box5だけが新規追加分)。

払戻2万円以上のレースを除外した回収率(cap後)を主判定、キャップ無しを参考併記する
(jra_eval.pyのmax_payout引数、市場ベンチマーク・等重み・in-sample最良パターン・
Nested LOBO OOF・選択バイアス診断・ブートストラップの全てに同一ルールで適用)。

母集団は既存のbox5/4/3・Step1・box2〜4探索で複数回使われてきた「使い切り」の211レースのまま
(jra_dataset.py)。box2〜4では全サイズでNested LOBO OOFが基準未達(box4のみ市場差+8.1ptだが
選択バイアス診断のtrue_edgeはマイナス)という結果だったため、box5でも同じ厳格な判定基準
(Nested LOBO OOF + 選択バイアス診断)を主判定とする。

出力: jra_search_box2345_umaren_cap_2026_08_23_result.json / _report.txt (data/jra_pipeline、研究ログ)。
"""
import json
import sys
...
N_PATTERNS = 1000
SEED = 2309  # box234版と同一(box2/3/4の再現性を保つ)
BOX_NS = (2, 3, 4, 5)
BETS = ["馬連"]
MAX_PAYOUT = 20000.0  # ユーザー依頼: 払戻2万円以上のレースは除外

OUT_JSON = DATA_DIR / "jra_search_box2345_umaren_cap_2026_08_23_result.json"
OUT_TXT = DATA_DIR / "jra_search_box2345_umaren_cap_2026_08_23_report.txt"
...
log("※この母集団はbox5/4/3・Step1・box2〜4探索で既に繰り返し使われている「使い切り」の母集団。")
