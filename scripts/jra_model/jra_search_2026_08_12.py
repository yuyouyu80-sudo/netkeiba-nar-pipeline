# -*- coding: utf-8 -*-
"""ユーザー依頼(2026-08-12): box4(pattern19)/box3(pattern95)の現行本番モデルが新規72レース
(0801以降)で市場割れしていた件の根本原因調査で、winner_box4.json/winner_box3.jsonの`method`
フィールド自体に「105レース中53レースのみのtrain/holdout 2分割検証、Nested LOBO OOF・選択
バイアス診断は未実施」と明記されていることが確認された。統計学者ペルソナのレビューで
「現行box4/box3の重み自体を、177レース全体でNested LOBO OOF+選択バイアス診断(ratio≥2.0
ゲート)にかけて再評価すべき」と提言されたため、本スクリプトでそれを実施する。

あわせて、2026-08-11の専門家レビューで「surf_ketto/surf_jockeyは既存シグナル(sire/bms/jt)と
高相関のはずという除外仮定が実データ(相関係数0.03〜0.31)で否定された」と判明したことを受け、
jra_signals.pyに追加した候補シグナル第2弾(ketto_training/ketto_comment/odds_jockey/surf_jt/
jockey_owner/prevjockey、計6個)も探索プールに含める。既存10シグナル+候補9(2026-08-11)+
候補6(2026-08-12)=計25シグナルでbox_n=5/4/3それぞれ独立に重みを再探索する。
...
OUT_JSON = DATA_DIR / "jra_search_2026_08_12_result.json"
OUT_TXT = DATA_DIR / "jra_search_2026_08_12_report.txt"
