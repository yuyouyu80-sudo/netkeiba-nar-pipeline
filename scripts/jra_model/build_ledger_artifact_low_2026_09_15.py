# -*- coding: utf-8 -*-
"""単勝回収率10%未満・複勝回収率25%未満・ワイド回収率10%未満の「消し材料」台帳3券種ぶんの
インタラクティブHTMLを`jra_ledger_search_low_2026_09_15.py`の出力から生成する。

デザインは既存の3台帳(2026-09-07生成、青系アクセント・「逆方向」eyebrow)を踏襲しつつ、
2026-09-15の設計変更(旧169atom「低い方」専用語彙 -> 単勝〜3連単110%台帳と共通の290atom
候補プールを流用し探索方向だけ反転)を反映した本文に更新する。HTML構造・AND再計算エンジン・
馬レコード圧縮は`build_ledger_artifact_2026_09_15.py`と共通のヘルパーをそのまま再利用する。
"""
import copy
import json
import sys
...
import build_factor_artifact as BFA  # noqa: E402
import build_ledger_artifact_2026_09_15 as BLA  # noqa: E402 (build_race_data_blob等を再利用)
...
SEARCH_JSON = DATA_DIR / "jra_ledger_search_low_2026_09_15_result.json"
FACTOR_JSON = DATA_DIR / "factor_database.json"

# 券種: (slug, combo_k, ordered, ファイル名, 既存Artifact URL, 対応する110%台帳(順方向)のURL)
BET_META = {
    "単勝": dict(slug="tansho_low", k=1, ordered=False, file="ledger_tansho_low.html",
                url="https://claude.ai/code/artifact/700851e2-e4c6-44cf-a1a0-f08f2b52186b",
                forward_url="https://claude.ai/code/artifact/4b9bba27-4c29-4942-903f-743cf68db35a"),
    "複勝": dict(slug="fukusho_low", k=1, ordered=False, file="ledger_fukusho_low.html",
    ...
    "ワイド": dict(slug="wide_low", k=2, ordered=False, file="ledger_wide_low.html",
                 url="https://claude.ai/code/artifact/d6b0babd-c8b4-404e-b6d7-6e59d90b6379",
                 forward_url="https://claude.ai/code/artifact/3e774502-09c8-4c39-8fa7-b22fa90c79df"),
}
...
<div class="page">
  <div class="eyebrow">JRA予想モデル・探索結果(逆方向)</div>
  <h1>__H1__</h1>
  <p class="dek">
  ...
  <div class="warnbar">
    ...
    <span>
      この一覧は「あえて低的中率・低回収率を狙う」逆方向の探索結果です。実際にこの条件に該当する馬を
      <b>本命として買う</b>ためのものではなく、<b>消し材料(軸から外す判断材料)の参考</b>として使うことを
      想定しています。的中率0.0%(対象レースで1度も的中していない)のパターンが大半を占め、これは
      強い事後選択バイアスの結果でもあります。95%信頼区間を検証していないため、「必ず外れる」と
      解釈しないでください。
    </span>
  </div>

  <div class="rebuild-note">
    <b>2026-09-15再構築</b>: 旧台帳(2026-09-07生成)は専用の「低い方」169atom語彙(通常の
    候補atomとは別の逆方向専用の語彙)を使っていましたが、その語彙を復元・生成したビームサーチ
