# -*- coding: utf-8 -*-
"""予想3頭軸流し(軸+相手2頭)モデルの確信度フィルタリング検証(5〜10レース/日)。
...
3連単(軸流し/マルチ)の7区分、軸=スコア1位、相手=スコア2〜3位)を使う。

2026-08-21の重み探索(jra_axis_search_2026_08_21.py)の結論: 軸流し専用の新規500パターン
探索はREJECTED(選択バイアス診断のtrue_edge/sd比が採否ゲート2.0を大幅未達)。現行box3重み
(winner_box3.json)をそのまま軸流しに転用した場合も、全211レースでは市場を+18.34pt上回るが、
重み自体のfit母集団(105レース)を除外した公正な評価(106レース)では市場超過が消え、
点推定はむしろマイナス(-13.63pt、95%CI=[-45.10,+21.93]ptで0をまたぐ)に転じる。
...
CURRENT_WEIGHT_FILE = "winner_box3.json"
SEARCH_RESULT_FILE = "jra_axis_search_2026_08_21_result.json"
OUT_CSV = DATA_DIR / "confidence_sweep_axis3.csv"
