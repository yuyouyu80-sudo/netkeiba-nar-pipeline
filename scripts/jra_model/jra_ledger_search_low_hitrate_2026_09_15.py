# -*- coding: utf-8 -*-
"""馬連・3連複の「外し(消し材料)」台帳を新規に、的中率5%未満基準でビームサーチ構築する。

単勝・ワイド(jra_ledger_search_low_2026_09_15.py)は回収率基準、複勝
(jra_ledger_search_fukusho_hitrate_2026_09_15.py)は的中率基準に変更済みだったが、
馬連・3連複はこれまで「外し」台帳自体が存在しなかった(既存110%台帳のみ)。
ユーザー指定により、最初から的中率(的中R÷対象R)5%未満を基準に新規構築する。

候補プール・母集団・coverage>=40%・ホールドアウト設計は既存の消し材料台帳と共通
(81グループ・290atom、直近2開催日を除いた561レース)。券種はr_size違いのみ
(馬連=2頭・3連複=3頭、いずれも着順不問)。
"""
import json
import sys
import time
...
def main():
    ...
    for bt, (r_size, ordered) in BET_SPECS.items():
        t0 = time.time()
        min_horses = r_size
        log(f"=== {bt} 探索開始(外し・消し材料、的中率{HIT_RATE_THRESHOLD_PCT:.0f}%未満) ===")
        frontier = []
        found = {}
        n_evals = 0
        for atom in atoms:
