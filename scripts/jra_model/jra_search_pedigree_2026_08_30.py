# -*- coding: utf-8 -*-
"""血統(種牡馬)ベースの候補シグナル第5弾(ped_sire_distance/surface/going/course、
jra_pedigree_features_2026_08_30.py・jra_signals.py CANDIDATE_SIGNALS_V5参照)を、
既存25シグナル(LEGACY+V2+V3、box_n=5/4/3独立)に追加した29シグナルで
Nested LOBO OOF + 選択バイアス診断にかける。

手法はjra_search_2026_08_12.py(surf_ketto/jockey候補6個の追加検証)を完全踏襲する
(N_PATTERNS=1000のDirichlet探索→in-sample最良→Nested LOBO OOF→選択バイアス診断→
採否ゲート、box_n=5/4/3それぞれ独立)。

前提: この時点でjra_pedigree_theory_2026_08_29/30.pyの記述的検証ではインブリード係数・
ニックスとも信号なしと判定済み([[project_jra_pedigree_theory_verification_2026_08_29]])。
本スクリプトが検証するのはそれとは別の血統由来シグナル(種牡馬の距離/表面/馬場/コース別
時点参照勝率)であり、netkeiba既存の`sire`シグナル(単一の総合勝率)より粒度が細かい点が
未検証だったため追加した。
"""
import json
import sys
...
OUT_JSON = DATA_DIR / "jra_search_pedigree_2026_08_30_result.json"
OUT_TXT = DATA_DIR / "jra_search_pedigree_2026_08_30_report.txt"
...
NAMES = JS.ALL_SIGNALS_V5  # 既存25 + 血統候補4(2026-08-30) = 29シグナル
log(f"レース数: {len(races)}  日付: {data['dates']}  頭数: {sum(len(r['df']) for r in races)}")
log(f"探索対象プール({len(NAMES)}シグナル): {NAMES}")

# ped_sire_*列が実際にマージされているか(jra_dataset.pyのPEDIGREE_FEATURES_CACHE経由)確認
sample_df = races[0]["df"]
has_ped_cols = all(f"ped_sire_{d}_runs" in sample_df.columns for d in ("distance", "surface", "going", "course"))
log(f"血統特徴量の列がマージ済みか: {has_ped_cols}")
if has_ped_cols:
    for dim in ("distance", "surface", "going", "course"):
        col = f"ped_sire_{dim}_runs"
        nonzero_races = sum(
            (pd.to_numeric(r["df"].get(col), errors="coerce").fillna(0) > 0).sum() for r in races
        )
        total_horses = sum(len(r["df"]) for r in races)
        log(f"  ped_sire_{dim}: 事前実績あり(runs>0) {nonzero_races}/{total_horses} "
...
for k in ("ped_sire_distance_win", "ped_sire_surface_win", "ped_sire_going_win", "ped_sire_course_win"):
    log(f"  prior[{k}] = {priors_fresh.get(k)}")
...
for BOX_N in BOX_NS:
    ...
    W_CURRENT = wvec(winner["weights"])  # 血統候補分は0(現行モデルは血統候補なし)
    ...
    ped_weight_share = sum(w for n, w in zip(NAMES, W_POOL[:, best_full]) if n in JS.CANDIDATE_SIGNALS_V5)
    log(f"\n[血統候補シグナルの寄与] 最良パターンにおける血統候補4個(2026-08-30)合計重み: "
        f"{ped_weight_share:.3f} ({ped_weight_share*100:.1f}%)")
    for n in JS.CANDIDATE_SIGNALS_V5:
        idx = NAMES.index(n)
    ...
    results_by_box[BOX_N] = {
        ...
        "best_full_population": {
            ...
            "ped_signal_weight_share": float(ped_weight_share),
...
for BOX_N in BOX_NS:
    ...
    log(f"box_n={BOX_N} (現行pattern{r['pattern_id_current']}): "
        ...
        f"血統候補寄与={r['best_full_population']['ped_signal_weight_share']*100:.1f}%")
