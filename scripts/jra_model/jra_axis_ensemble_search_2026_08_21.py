# -*- coding: utf-8 -*-
"""Front2: axis5/4/3(1頭軸流し買い)のアンサンブル(重みブレンド)+時系列検証(2026-08-21新設)。
box用jra_ensemble_search_2026_08_21.pyと対をなす軸流し版。

Front1(jra_axis_search500_2026_08_21_v3signals.py)は30シグナルプールでaxis5/4/3を独立に
500パターン探索したが、全てREJECTEDだった。Nested LOBO OOFのfold毎選択パターンがユニーク
2〜3/36と強く退化しており、単一パターンのin-sample選択が不安定であることを示唆する。
本スクリプトは同一の500パターン母集団(同一SEED)から多様性を考慮した上位8パターンの
ブレンドを作り、選択の不安定さを緩和できるか検証する。設計はbox版と同一(4候補比較・
selection_optimism流用・chronological_oofはロバスト性チェックのみ)。

出力: jra_axis_ensemble_search_2026_08_21_result.json / _report.txt (data/jra_pipeline、
git管理下、研究ログ)。本番重みへの反映は専門家レビューを経てユーザーが判断する。
"""
import json
import sys
...
N_PATTERNS = 500
SEED = 2852  # Front1(jra_axis_search500_2026_08_21_v3signals.py)と同一シード・同一母集団
BOX_NS = (5, 4, 3)
CURRENT_BOX_WEIGHT_FILES = {5: "winner_v3.json", 4: "winner_box4.json", 3: "winner_box3.json"}
TOP_N, MAX_COS_SIM = 8, 0.9
...
OUT_JSON = DATA_DIR / "jra_axis_ensemble_search_2026_08_21_result.json"
OUT_TXT = DATA_DIR / "jra_axis_ensemble_search_2026_08_21_report.txt"
...
for BOX_N in BOX_NS:
    ...
    log(f"市場(軸=1番人気)  ワイド={mkt['model']:.2f}%")
    log(f"現行box重み転用  ワイド={r_current['model']:.2f}%  市場差={r_current['excess']:+.2f}pt")
    ...
    blend_picks_full = AE.score_picks(mats_all, w_blend_full, BOX_N)
    ...
    log(f"[全{len(races)}レースでのin-sampleアンサンブルブレンド] "
        ...
        f"ワイド={r_blend_full['model']:.2f}%(市場差={r_blend_full['excess']:+.2f}pt)")
    ...
    log(f"\n[Nested LOBO OOF・単一ベスト] ワイド={oof_single['model']:.2f}%  "
        f"市場差={oof_single['excess']:+.2f}pt  "
        f"ユニークpattern {oof_single['n_unique_patterns']}/{oof_single['n_folds']}")
    log(f"[Nested LOBO OOF・アンサンブルブレンド] ワイド={oof_ensemble['model']:.2f}%  "
        f"市場差={oof_ensemble['excess']:+.2f}pt  "
        f"ユニーク選択集合 {oof_ensemble['n_unique_patterns']}/{oof_ensemble['n_folds']}")

    W_small = np.column_stack([w_single_best_full, w_blend_full, W_CURRENT_BOX, W_EQUAL])
    opt = AE.selection_optimism(ev, mats_all, W_small, n_rep=200, seed=2872)
    log(f"\n[選択バイアス診断(4候補: 単一ベスト/ブレンド/現行box重み/等重み)] ブロック半分割×200反復:")
    ...
    log(f"\n[時系列walk-forward(参考・ロバスト性チェック専用)] "
        f"単一ベスト: ワイド={coof_single['model']:.2f}%  市場差={coof_single['excess']:+.2f}pt  "
        f"({coof_single['n_folds']}fold、対象{len(coof_single['tested_race_idx'])}レース)")
    log(f"  アンサンブルブレンド: ワイド={coof_ensemble['model']:.2f}%  "
    ...
    log(f"  → LOBOとwalk-forwardの市場差の符号一致: "
    ...
    log(f"  現行box重み転用比の差 95%CI[{boot_ens_vs_current['lo']:+.2f}, "
        f"{boot_ens_vs_current['hi']:+.2f}]pt(採否判定の本命指標)")
    ...
    log(f"\n採否ゲート(現行box重み転用比95%CI下限>0): {decision}")

    results_by_box[BOX_N] = {
        "market": mkt["model"],
        "current_box_weight_reused": {"model": r_current["model"], "excess": r_current["excess"]},
...
for BOX_N in BOX_NS:
    ...
    log(f"axis{BOX_N}: 単一ベストOOF市場差={r['nested_lobo_oof_single']['excess']:+.2f}pt"
        ...
        f"現行box重み比CI=[{bc['lo']:+.2f},{bc['hi']:+.2f}]pt  "
