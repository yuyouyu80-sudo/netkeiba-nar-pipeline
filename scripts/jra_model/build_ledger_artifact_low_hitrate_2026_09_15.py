# -*- coding: utf-8 -*-
"""馬連・3連複の「外し(消し材料)」台帳(既存Artifact e156a3dc・249e12c1、これまで
「回収率10%未満」基準・169atom専用語彙)を、単勝・ワイド版と共通の290atom候補プール+
的中率5%未満基準へ切り替えて再生成する(jra_ledger_search_low_hitrate_2026_09_15.pyの
出力を使用)。デザイン・文言は複勝版(build_ledger_artifact_fukusho_hitrate_2026_09_15.py)を
踏襲し、券種依存部分(COMBO_K・110%台帳へのリンク・警告文の的中定義)だけ差し替える。
"""
import json
import sys
...
SEARCH_JSON = DATA_DIR / "jra_ledger_search_low_hitrate_2026_09_15_result.json"
FACTOR_JSON = DATA_DIR / "factor_database.json"

BET_META = {
    "馬連": dict(k=2, ordered=False, file="ledger_umaren_hitrate.html",
                url="https://claude.ai/code/artifact/e156a3dc-f78b-4be5-8679-c8a5615c2d44",
                forward_url="https://claude.ai/code/artifact/88c905f2-13cb-4a06-b2b0-cd1790c52fb4",
                hit_desc="上位2頭の組番に一致していない"),
    "3連複": dict(k=3, ordered=False, file="ledger_sanrenpuku_hitrate.html",
                 url="https://claude.ai/code/artifact/249e12c1-f2d9-4af7-8032-2a1897802eb6",
                 forward_url="https://claude.ai/code/artifact/5aac2d4a-5cb6-4418-9a1e-fce73f867971",
                 hit_desc="上位3頭の組番に一致していない"),
}

NOTES_BY_BT = {
    "馬連": "馬連は2頭の組番(着順不問)を当てる券種のため、対象馬を絞っても組み合わせ数"
           "(点数)は該当馬数k頭に対しk×(k-1)÷2で増えます。この一覧は「めったに上位2頭の"
           "組番に一致しない」条件の組み合わせを的中率の低さで直接ランキングしたものです。",
    "3連複": "3連複は3頭の組番(着順不問)を当てる券種のため、対象馬を絞っても組み合わせ数"
            "(点数)が急増しやすい券種です。この一覧は「めったに上位3頭の組番に一致しない」"
...
<div class="page">
  ...
  <p class="dek">
    ...
    的中率が低い順に全件収録したものです。単勝・複勝・ワイド版に続き、__BT__も同じ290atom
    候補プール・的中率5%未満基準で新規構築しています(理由は下の再構築ノート参照)。
  ...
  <div class="warnbar">
    ...
    <span>
      ...
      <b>__BT__の本命として買う</b>ためのものではなく、<b>消し材料(軸から外す判断材料)の参考</b>として
      使うことを想定しています。的中率0.0%(対象レースで一度も__HITDESC__)のパターンが大半を占め、
      これは強い事後選択バイアスの結果でもあります。95%信頼区間を検証していないため、
      「必ず外れる」と解釈しないでください。回収率列も参考として併記していますが、この一覧の
      並び替え・絞り込みの基準はあくまで的中率です。
    </span>
  </div>

  <div class="rebuild-note">
    <b>2026-09-15新規構築</b>: 旧台帳(2026-09-12生成、169atom専用「低い方」語彙・回収率10%未満基準)を、
    単勝・複勝・ワイド版と共通の290atom候補プール(jra_ledger_search_low_hitrate_2026_09_15.py)を
    使い、判定基準も「回収率」から「的中率(的中R÷対象R)が5%未満」に変更して再構築しました
    (複勝で回収率基準が5件まで激減した経緯を踏まえ、__BT__は最初から的中率基準で探索)。
...
(function () {
  ...
  document.getElementById("footer").textContent =
    `jra_ledger_search_low_hitrate_2026_09_15.py / build_ledger_artifact_low_hitrate_2026_09_15.py — 生成日時: ${meta.generated_at}`;
