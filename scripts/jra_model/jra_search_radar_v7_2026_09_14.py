# -*- coding: utf-8 -*-
"""box3/4/5の候補シグナルプールをLEGACY10+V1-V4=41+V5=4+V6=1+レーダー複合9(V7)=55本に
統合して、Nested LOBO OOF + 選択バイアス診断にかける。
...
以下が判明した:
  ...
  2. (2026-09-15訂正、Opus5レビューで指摘) 当初「CANDIDATE_SIGNALS_V4は2026-08-28の
     単独検証以降、統合探索チェーンに一度も合流していなかった」としていたが誤り。
     実際にはjra_search_boxN_2026_09_11.pyが同一319レースでNAMES=ALL_SIGNALS_V5+
     CANDIDATE_SIGNALS_V4という45本プール(LEGACY+V1-V5、N=2000)を既に検証済み
     (data/jra_pipeline/jra_search_box{3,4,5}_2026_09_11_result.json)。正しくは
     「V4はjra_signals.pyのALL_SIGNALS_V5/V6という**モジュール定数チェーン**
     (ALL_SIGNALS=LEGACY+V1+V2+V3から分岐しておりV4を経由しない)には合流していなかった」
     という限定的な話で、V4自体は2026-09-11時点でV5とは統合済みだった。
ユーザーはAskUserQuestionで「V4も合流させ全55本を統合」を選択したため、本探索では
V4・V6・V7(レーダー)を初めて一緒にモジュール定数チェーン上で検証する。
...
【2026-09-15追記: Opus5サブエージェントによるレビュー結果】
結論(3box全て不採用・現行モデル無変更)自体は独立検算・追加のCI計算により
むしろ補強されたが、以下の重要な限界が判明した(詳細はメモリ
project_jra_box345_search_v7_unified_2026_09_14.md参照):
  - **Dirichlet(1)^55×2000本という設計は、55次元では構造的にスパースな重み配分
    (少数シグナルへの集中)を生成できない**(理論上有効シグナル数E[1/Σw²]=28本、
    実測mean28.4)。N_PATTERNSを1000→2000に増やしても効果はゼロだった(box3の
    最良パターン#202は最初の1000本の中に既に含まれていたことを確認)。現行モデル
    (LEGACY10本に100%集中)と同種の重みベクトルは2000本中1本も存在しない
    (LEGACY合計重みの実測maxは0.421)。そのため「現行モデルが2000パターン中
    97〜100パーセンタイル」は「現行10本集中モデルが、55本の希薄なブレンド群より
    強い」以上のことを意味しない。
  - 現行モデル自体も319レース中105レース(fitted_on.search_dates+holdout_dates=
    20260711〜0726)で選定済みのため、**真に未見の214レースでの現行モデル市場差は
    box5=-1.26pt/box4=+4.57pt/box3=-0.19pt**(全て95%CIが0を跨ぐ、2026-09-15に
    verify_truly_unseen.pyで独立検算・一致確認済み)であり、全319レース値
    (+8.66/+25.83/+7.89pt)ほど強くない。
  - レーダー複合9本のうちradar_jt_base(=jtのみ)とradar_form_agari(=agariのみ)は
    単一メンバーカテゴリのため既存シグナルの完全な複製(Pearson相関+1.000)で、
    候補プールに実質的な重複を持ち込んでいた。
  - Nested LOBO OOFがbox4で2/54、box3で5/54ブロックしかユニークな重みパターンを
    選んでおらず、「探索手続き全体を交差検証した」という主張はbox4ではほぼ成立して
    いなかった(n_unique_patternsを記録していなかったことも指摘、09-11版スクリプトは
    記録していた)。
本スクリプト自体は変更していない(結果の再現性を保つため)。上記の限界は解釈時に
必ず考慮すること。次回同種探索を行う場合は、alpha<1のDirichletや現行重み近傍の
局所探索アームの併用、n_unique_patterns記録の復活を検討する。
"""
import json
import sys
