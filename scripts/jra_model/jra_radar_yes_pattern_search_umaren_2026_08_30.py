# -*- coding: utf-8 -*-
"""jra_radar_yes_pattern_search_2026_08_29.py の**目的券種を「馬連」のみに差し替えた**
サイドカー(ユーザー明示依頼、2026-08-30)。

**位置づけ(重要、元スクリプトと同じ)**: これは採用候補を選ぶための探索ではない。
  (軸統合レベル 8/7/6/5) × (box_n 3/4/5) × (集約方法 幾何平均/最小値/上位3軸和/面積[全順序])
の全空間を総当たりし、名目上のPASS(lo>0)を隠さず全部列挙する。そのうえでブロック半分割の
選択バイアス診断(n_rep=200、seed固定)を必ず併走させ、見つかったPASSが「真に汎化する」のか
「探索アーティファクト」なのかを両論併記する。名目PASSをそのまま採用可能な発見として
報告してはならない。

元スクリプト(2026-08-29)との差分は**目的券種のみ**:
  * 目的関数 bets = ["馬連"] (元は JE.OBJ_BETS = ["複勝", "ワイド"])
  * 全ての cost_weighted_rate / block_bootstrap_diff 呼び出しに bets=OBJ_BETS を明示
  * 出力ファイル名
picks生成ロジック(market_picks / 線形・幾何・最小・top3sum・面積)は券種に依存しないため
一切変更していない。モデル計算側(jra_radar_categories.py / jra_signals.py / jra_eval.py /
jra_backtest.py)も一切改造しない。
...
計算上の工夫(元スクリプトと同じ):
  * ブートストラップはブロック単位・比推定量なので、事前に「候補×ブロック」の賭金和・払戻和を
    ...
    リサンプルをブロックの出現回数ベクトルに変換して掛けるだけ)。等価性は run 時に
    ev.block_bootstrap_diff(bets=["馬連"]) と突き合わせて検証する(本版では (レベル,box) ごとに
    スカラー法1件+面積1件の計2件を検証する = 元版の12点から24点に強化)。
...
def main():
    ...
    breakeven = {bn: JE.breakeven_pct(bn, bets=OBJ_BETS) for bn in BOX_NS}
    print(f"  レース数(除外前)={len(races_all)}  目的券種={OBJ_BETS}  ({time.time()-t0:.1f}s)")
    print(f"  理論ブレークイーブン(馬連)={breakeven}")
    ...
    for level_name, cat_map in LEVELS.items():
        ...
        for box_n in BOX_NS:
            ...
            # --- ベクトル化ブートストラップが本家(bets=["馬連"])と一致することを毎回2件検証する
            for chk_key in [(SCALAR_METHODS[0], None), ("area", 0)]:
                vi = labels.index(chk_key)
                ref = ev.block_bootstrap_diff(picks_cache[chk_key], lin_picks, bets=OBJ_BETS)
                ok = (abs(ref["lo"] - lo_d[vi]) < 1e-9 and abs(ref["hi"] - hi_d[vi]) < 1e-9
                      and abs(ref["mean"] - mean_d[vi]) < 1e-9)
                verify_notes.append({
                    "level": level_name, "box_n": box_n,
                    "checked": f"{chk_key[0]}|order_idx={chk_key[1]}",
            ...
            for i, (m, oi) in enumerate(labels):
                ...
                row = {
                    ...
                    "market_rate_pct": mkt_rate, "breakeven_pct": breakeven[box_n],
            ...
            print(f"  box{box_n}: 線形={lin_rate:.2f}%(市場={mkt_rate:.2f}%, BE={breakeven[box_n]:.2f}%) "
    ...
    n_bad = int(sum(1 for v in verify_notes if not v["match"]))
    print(f"ベクトル化ブートストラップ検証: {len(verify_notes)}点中 不一致={n_bad}")
    ...
    if passes.empty:
        ...
        for _, r in near.iterrows():
            print(f"  {r['level']:32s} box{r['box_n']} {r['method']:>9s} "
                  f"order_idx={r['order_idx']} lin={r['linear_rate_pct']:6.2f}% "
                  f"cand={r['candidate_rate_pct']:6.2f}% diff={r['diff_pt']:+6.2f}pt "
    ...
    out = {
        "config": {
            ...
            "obj_bets": OBJ_BETS,
            "breakeven_pct_by_box": breakeven,
            "baseline_script": "jra_radar_yes_pattern_search_2026_08_29.py (bets=複勝+ワイド)",
        },
    ...
    (OUT_DIR / "jra_radar_yes_pattern_search_umaren_2026_08_30_result.json").write_text(
        json.dumps(out, ensure_ascii=False, indent=2, default=str), encoding="utf-8", newline="\n"
    )
    df.to_csv(OUT_DIR / "jra_radar_yes_pattern_search_umaren_2026_08_30_all_candidates.csv",
              index=False, encoding="utf-8")
    print(f"\nwrote jra_radar_yes_pattern_search_umaren_2026_08_30_result.json / _all_candidates.csv "
          f"({time.time()-t0:.1f}s)")
