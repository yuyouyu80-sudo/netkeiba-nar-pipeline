# -*- coding: utf-8 -*-
"""複勝の消し材料台帳(既存Artifact 20c86c67、これまで「回収率25%未満」基準)を
「的中率5%未満」基準へ切り替えて再生成する。回収率25%未満基準では該当が5件まで
激減したため(2026-09-15、jra_ledger_search_low_2026_09_15.py)、ユーザー確認の上で
的中率(的中R/対象R)を基準にした試算(jra_ledger_search_fukusho_hitrate_trial_2026_09_15.py、
242パターン)を行い、この本番版(jra_ledger_search_fukusho_hitrate_2026_09_15.py)で
確定した結果をArtifact化する。

HTML構造・AND再計算エンジン・馬レコード圧縮はbuild_ledger_artifact_2026_09_15.pyの
ヘルパーを再利用。CSS/文言はbuild_ledger_artifact_low_2026_09_15.py(青系アクセント・
「消し材料」の位置づけ)を踏襲しつつ、基準が回収率でなく的中率である点を全文言で
明示する。
"""
import json
import sys
...
import build_ledger_artifact_2026_09_15 as BLA  # noqa: E402 (build_race_data_blob再利用)
...
SEARCH_JSON = DATA_DIR / "jra_ledger_search_fukusho_hitrate_2026_09_15_result.json"
FACTOR_JSON = DATA_DIR / "factor_database.json"

BT = "複勝"
URL = "https://claude.ai/code/artifact/20c86c67-c718-4039-97ce-88b4ab509ad3"
FORWARD_URL = "https://claude.ai/code/artifact/2297a9a1-8cd5-4386-8bae-e04fc2e88a4c"
OUT_FILE = "ledger_fukusho_hitrate.html"
...
<style>
  ...
  @media (prefers-color-scheme: dark) {
    :root:not([data-theme="light"]) {
      ...
      --accent: #7C9CDE; --accent-ink: #111A2E; --accent-soft: #26314C; --accent-soft-ink: #C3D2F2;
  ...
  * { box-sizing: border-box; }
  ...
  .dek { color: var(--ink-muted); font-size: 14px; max-width: 76ch; margin: 0 0 6px; }
  ...
  .and-panel .and-stat .value.accent { color: var(--accent); }
  ...
  .return-rate { font-family: var(--mono); font-variant-numeric: tabular-nums; }
  .hit-rate { font-family: var(--mono); font-weight: 700; color: var(--accent); font-size: 14px; }
...
<div class="page">
  <div class="eyebrow">JRA予想モデル・探索結果(逆方向・的中率基準)</div>
  <h1>複勝的中率5%未満台帳</h1>
  <p class="dek">
    <a href="__FORWARD_URL__">複勝回収率110%台帳</a>の逆方向のレポートです。
    ...
    <b>的中率(的中R÷対象R)が5%未満</b>になる条件の組み合わせをビームサーチで探索し、
    的中率が低い順に全件収録したものです。当初は単勝・ワイド版と同じ「回収率が低い」基準
    (25%未満)で探索しましたが該当が5件まで激減したため、複勝は「回収率」でなく
    「的中率」を基準に切り替えています(理由は下の再構築ノート参照)。日次の予想レポートとは
    別物で、検証済みの結論を示すものではありません。
  </p>
  <div class="cross-link">← <a href="__FORWARD_URL__">複勝回収率110%台帳(順方向)</a> ／
  ...
  <div class="warnbar">
    ...
    <span>
      この一覧は「あえて低的中率を狙う」逆方向の探索結果です。実際にこの条件に該当する馬を
      <b>複勝の本命として買う</b>ためのものではなく、<b>消し材料(軸から外す判断材料)の参考</b>として
      使うことを想定しています。95%信頼区間を検証していないため、「必ず3着以内に入らない」と
      解釈しないでください。回収率列も参考として併記していますが、この一覧の並び替え・絞り込みの
      基準はあくまで的中率です。
    </span>
  </div>

  <div class="rebuild-note">
    <b>2026-09-15基準変更</b>: 旧台帳(2026-09-07生成、169atom専用語彙)は「回収率25%未満」を
    基準にしていましたが、単勝・ワイド版と共通の290atom候補プールに切り替えたところ
    (jra_ledger_search_low_2026_09_15.py)該当パターンが5件まで激減しました。これは290atomプールが
    「上位N位以内」系の条件主体で、複勝(3着以内で的中)を回収率ベースで低く抑える組み合わせが
    見つかりにくいためです。そこでユーザー確認の上、判定基準を「回収率」から
    「的中率(的中R÷対象R)が5%未満」に変更して再探索したところ242パターンまで回復しました。
    探索母集団は直近2開催日をホールドアウトとして除いた__SEARCHPOP__レース(全__FULLPOP__レース中)、
    候補プールは__NGROUPS__グループ・__NATOMS__atomで、単勝・ワイド版・110%台帳と共通です。
  </div>

  <div class="toolbar">
    <input type="search" id="filter-input" placeholder="条件で絞り込み…(例: 人気、休み明け、輸送)">
    ...
    <table id="report-table">
      <thead>
        <tr>
          ...
          <th data-key="hit_rate_pct" class="sorted">的中率<span class="arrow">▾</span></th>
          <th data-key="return_rate_pct">回収率(参考)<span class="arrow">▾</span></th>
...
(function () {
    ...
    if (result.n_races === 0) {
      ...
      statsHtml = `
        <div class="and-stats">
          ...
          <div class="and-stat"><div class="label">的中率</div><div class="value accent">${result.hit_rate_pct.toFixed(1)}%</div></div>
          <div class="and-stat"><div class="label">回収率(参考)</div><div class="value">${result.return_rate_pct.toFixed(1)}%</div></div>
        </div>
  ...
  document.getElementById("notes-block").innerHTML = `
    ...
    <p>複勝は3着以内(小頭数レースでは2着以内)に入れば的中する券種のため、単勝・ワイド版のような
    的中率0.0%のパターンはほとんど見つかりません。この一覧は「めったに3着以内に入らない」
    条件の組み合わせを的中率の低さで直接ランキングしたものです。</p>
  `;

  document.getElementById("footer").textContent =
    `jra_ledger_search_fukusho_hitrate_2026_09_15.py / build_ledger_artifact_fukusho_hitrate_2026_09_15.py — 生成日時: ${meta.generated_at}`;
  ...
  function render() {
    ...
    tbody.innerHTML = filtered.map(r => {
      ...
      return `<tr${rowCls ? ` class="${rowCls}"` : ""} data-rank="${r.rank}">
        ...
        <td class="num hit-rate">${r.hit_rate_pct.toFixed(2)}%</td>
...
def main():
    ...
    title = "複勝的中率5%未満台帳"
    desc = ("複勝回収率110%台帳の逆方向: JRAファクター検証データベースと共通の"
            f"{search['meta']['n_atoms']}atom候補プールを使い、対象レース数が全体の40%以上・"
            "的中率が5%未満になる複勝パターンを的中率が低い順に全件収録(消し材料の参考、"
            "回収率基準では該当5件のみだったため的中率基準へ変更)")
    css_html = (CSS_TEMPLATE
                .replace("__TITLE__", title).replace("__DESC__", desc)
                .replace("__FORWARD_URL__", FORWARD_URL)
