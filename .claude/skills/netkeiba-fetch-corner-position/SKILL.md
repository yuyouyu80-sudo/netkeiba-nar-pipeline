...
description: 指定した開催日(YYYYMMDD)または単一race_idについて、netKeibaのnewspaper.htmlに含まれる「AI展開」4コーナー位置取り予想を取得し、既存のdata/newspaper/{race_id}.csvへcorner4_rank/corner4_gap_pct/corner4_gap_lengths/corner4_speedupを追加・上書きする。ユーザーが「4コーナー位置」「展開予想」「AI展開」の取得・更新を依頼した際に使う。事前にnetkeiba-fetch-newspaperで対象race_idのCSVが生成済みであることが前提。
---

# netkeiba-fetch-corner-position

このワークスペースで、`scripts/fetch_corner_position.py` を使って開催日1日分(または単一race_id)
の4コーナー位置取り予想(netkeibaの「AI展開」ウィジェット)データを取得し、既存の馬柱(newspaper)
CSVへ追加するスキル。
...
2. **結果確認**
   - `--date` 実行時は最後に `refreshed corner4 position: {更新頭数}/{総頭数} horses across
     {成功レース数}/{全体レース数} races for {YYYYMMDD}` のサマリ行が出力される。
